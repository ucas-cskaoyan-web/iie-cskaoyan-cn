...
const APP_BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH ?? "/__article-tools/ucas-course-sign-in";
const apiPath = (path: string) => `${APP_BASE_PATH}${path}`;
...
export default function Home() {
	...
	const getServerTimeOffset = async (): Promise<number> => {
		...
		try {
			...
			const res = await fetch(apiPath("/api/course-uuid/timestamp"), {
				cache: "no-store"
			});
			if (!res.ok) {
				throw new Error();
	...
	const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			const res = await fetch(apiPath("/api/course-uuid/query"), {
				method: "POST",
				headers: {
	...
	const onDirectSign = async () => {
		...
		try {
			...
			const res = await fetch(apiPath("/api/course-uuid/sign"), {
				method: "POST",
				headers: {
					"Content-Type": "application/json"
				},
